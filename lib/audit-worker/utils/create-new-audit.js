 ...
 */
export async function createNewAuditForConfig(config) {
    ...
    try {
        ...
        await saveReport({ ...transformed, site: config._id }, raw);
        await updateSite(config._id, { last_audit: new Date().toISOString(), thumbnail });
